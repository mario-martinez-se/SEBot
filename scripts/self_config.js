...
module.exports = (robot) => {
  ...
  robot.respond(/(.*)(?:config|configuration)(.*)/i, [], (res) => {
    res.send(`I have been told not to repeat myself for ${commons.getMutePeriod(robot.brain, res.message.room)} secs in this channel`);
  });
  ...
  robot.respond(/(?:hi|hello)/i, [], (res) => {
    res.send(`:wave:Hello! I am SEBot. I can provide extended information about Jira tickets and GitHub Pull Requests whenever I detect you are talking about them. I won't repeat the same for at least ${commons.getMutePeriod(robot.brain, res.message.room)} seconds, but if I get too much repetitive, you can just ask me "@sebot don't repeat for X minutes".`)
  });
